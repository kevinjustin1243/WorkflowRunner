...
- Postgres (the `postgres-data` named volume) is the source of truth for accounts, runs, and workflows. The `./server/workflows` bind-mount is a **read-only first-boot migration source** — `workflows` is auto-populated from the directory only when the table is empty. After that the DB wins. The `./server/accounts.json` and `./server/runs.json` mounts are commented out by default; uncomment them only when upgrading from a pre-Postgres install (must exist as files on the host before first boot, else Docker mkdir's them into directories).
